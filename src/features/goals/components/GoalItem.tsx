...
function GoalItem({ goal }: Props) {
  ...
  return (
    <div
      className={
        "flex justify-between items-center w-full bg-white p-3 rounded-lg text-lg shadow-sm border-l-8 border-primary-500 space-x-3"
      }
    >
      <Link href={`/dashboard/goals/${id}`} className="w-full">
        <p>{title}</p>
      </Link>
      <VerticalOptionsIcon
        options={[
